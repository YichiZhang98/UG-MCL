...
def test_all_case(net, base_dir, method="unet_3D", test_list="full_test.list", num_classes=4, patch_size=(48, 160, 160), stride_xy=32, stride_z=24, test_save_path=None):
    ...
    with open(test_save_path + "/{}.txt".format(method), "a") as f:
        for image_path in tqdm(image_list):
            ...
            for ii in range(4):
                total_metric[ii].append(metric[ii])
            f.writelines("{},{},{},{},{}\n".format(
    ...
    avgstd = []
    used_metrics = ['Dice', 'Jaccard', '95HD', 'ASD']
    for ii in range(4):
        avgstd.append([np.mean(total_metric[ii]),np.std(total_metric[ii])])
        print(used_metrics[ii], 'Avg:', avgstd[ii][0],'Std',avgstd[ii][1])
    
    return total_metric  #aa#total_metric / len(image_list)
